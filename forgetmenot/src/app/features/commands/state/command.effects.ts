        ...
        return this.commandsApiService.dispatchCommand(userInput).pipe(
          switchMap((result: any) => {
            if (result.function === 'createNote') {
              return [
                noteActionTypes.createNoteSuccess({ note: result.note }),
                commandActionTypes.dispatchCommandSuccess({ result: result }),
              ];
            }
            if (result.function === 'getNotes') {
              return [
                noteActionTypes.getNotesSuccess({ notes: result.notes }),
                commandActionTypes.dispatchCommandSuccess({ result: result }),
              ];
            }
